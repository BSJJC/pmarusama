    ...
    >
      ...
      >
        ...
        <span class="block text-sm font-bold md:text-lg"
          >YouTubeやTikTok等の動画投稿サイトで活動中のマルチエンターテイナー！</span
        >
        <span class="block text-sm font-bold md:text-lg"
          >癖になる短編アニメ動画が大人気で、現在YouTubeチャンネルの登録者数は255万人、動画総再生数は20億回を突破。</span
        >
      </div>
...
<style scoped>
#p-profile__main {
  background: url(@/assets/imgs/profile-bg-sp.png) left center/100% 100%
    no-repeat;
}

@media (min-width: 768px) {
  #p-profile__main {
    background: url(@/assets/imgs/profile-bg.png) left center/auto 105%
      no-repeat;
  }
}
...
@media (min-width: 1024px) {
  #p-profile__main {
    background: url(@/assets/imgs/profile-bg.png) left center / auto 100%
      no-repeat;
  }
}
